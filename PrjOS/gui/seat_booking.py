...
def show_seat_booking(root, username, movie_title):
        ...
        for j in range(5):
            ...
            if seat_id in booked_seats:
                if booked_seats[seat_id] == username:
                    btn.config(bg="red", state="normal", command=lambda s=seat_id: toggle_seat(s))
                else:
    ...
    def cancel_selected_seats():
        if selected_to_cancel:
            if update_id:
                root.after_cancel(update_id)
            show_payment_screen(root, username, movie_title, [], list(selected_to_cancel))
        else:
            messagebox.showwarning("No Seats", "Please select your booked seats to cancel.")

    tk.Button(btn_frame, text="Pay", font=("Arial", 12), width=10,
            command=pay_selected_seats).pack(side="left", padx=10)

    tk.Button(btn_frame, text="Cancel", font=("Arial", 12), width=10,
            command=cancel_selected_seats).pack(side="left", padx=10)
